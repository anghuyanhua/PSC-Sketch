...
public:
    ...
    void detectHeavyChange(int time_now){
        std::unordered_map<int64_t, int64_t> heavychanger;
        std::unordered_map<int64_t, int> time_interval;
        for (int i = 0; i < w; ++i){
            for (int j = 0; j < d; ++j){
                ...
                if (pre > 0 && cur > 0 && std::abs(pre - cur) >= 30){
                    ...
                    if (last_change_time == 0){
                        ...
                        last_change_time = time_now;
                        time_interval[id] = interval;
                    }
                }
            }
        }
        std::ofstream heavychangerFile("./PSCSketch_Estimation_Result/heavychanger_flow_" + std::to_string(time_now) + ".txt");
        for (const auto& pair : heavychanger) {
            heavychangerFile << "IP: " << pair.first << ", Spread: " << pair.second << "\n";
        }
        heavychangerFile.close();
        std::ofstream intervalFile("./PSCSketch_Estimation_Result/ip_time_interval_" + std::to_string(time_now) + ".txt");
        for (const auto& pair : time_interval) {
            intervalFile << "IP: " << pair.first << ", Interval: " << pair.second << "\n";
        }
        intervalFile.close();
    }
