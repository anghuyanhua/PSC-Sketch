#ifndef _TOPKSTRUCT_H_
#define _TOPKSTRUCT_H_
...
class Topkstruct {
public:
    Topkstruct(std::size_t w) : w(w), field(3), topkSeed(9) {
        topkstruct.resize(w);
    ...
    std::map<std::pair<uint64_t, uint64_t>, uint64_t> report() {
        ...
        std::ofstream file("../Strawman1_Bloomfilter_Countmin/Strawman1_Estimation_Result_File/topk_list.txt");
        for (const auto& item : sorted_topk_list) {
            if (item[0] != 0) { // Only write non-zero entries
                file << item[0] << ", " << item[1] << ", " << item[2] << "\n";
            }
        }

        for (const auto& item : sorted_topk_list) {
            if (item[2] > 1) { // 只考虑频率计数大于1的项目
